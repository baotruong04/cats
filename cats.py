...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def check(paragraph):
        words_list = remove_punctuation(paragraph).split()
        for i in range(len(words_list)):
            if words_list[i].lower() in subject:
                return True
        return False
    return check
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    len_to_loop = min(len(source_words), len(typed_words))
    correct_count = 0
    for i in range(len_to_loop):
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    res = [word for word in word_list if diff_function(typed_word, word, limit) <= limit]
    ...
    if not res:
        ...
        closest_word = min(res, key=lambda word: diff_function(typed_word, word, limit)) 
        return closest_word
    # END PROBLEM 5
...
def furry_fixes(typed, source, limit):
    ...
    if limit == 0:
        if typed == source:
            ...
            return 1
    elif typed == source[:len(typed)] or typed[:len(source)] == source:
...
def minimum_mewtations(typed, source, limit):
    ...
    3
    """
    if source == typed: # Base cases should go here, you may add more base cases as needed.
        # BEGIN
        return 0
        # END
    elif not len(source) or not len(typed):
        return abs(len(source) - len(typed))
    elif limit == 0:
        if source == typed:
            return 0
...
def report_progress(typed, source, user_id, upload):
    ...
    while i < len(typed) and not done:
        if typed[i] == source[i]:
            correct_count += 1
        else:
            done = True
...
def time_per_word(words, timestamps_per_player):
    ...
    for i in range(len(tpp)):
        each_player_duration = [tpp[i][j + 1] - tpp[i][j]for j in range(len(tpp[i]) - 1)]
        times.append(each_player_duration)
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    fpp = [[] for i in player_indices] #fastest_per_player 
    for i in word_indices:
        player_idx = min(player_indices, key=lambda idx: times[idx][i])
        fpp[player_idx].append(words[i])
    return fpp
    # END PROBLEM 10
